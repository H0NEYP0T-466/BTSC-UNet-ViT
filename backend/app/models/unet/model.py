"""
UNet model architecture for brain tumor segmentation.
"""
import torch
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"UNet initialized: in_channels={in_channels}, out_channels={out_channels}, features={features}", extra={
            'image_id': None,
            'path': None,
            'stage': 'model_init'
        })
...
def get_unet_model(
    ...
    logger.info(f"Created UNet model with {sum(p.numel() for p in model.parameters())} parameters", extra={
        'image_id': None,
        'path': None,
