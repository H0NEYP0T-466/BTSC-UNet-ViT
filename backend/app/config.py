...
class Settings(BaseSettings):
    ...
    CLAHE_CLIP_LIMIT: float = 1.5  # Reduced from 2.0 to 1.5 to preserve image details
    CLAHE_TILE_GRID_SIZE: tuple = (8, 8)
    MEDIAN_KERNEL_SIZE: int = 3
    NLM_H: int = 8  # Reduced from 10 to 8 for less blur
    UNSHARP_RADIUS: float = 1.0  # Reduced from 1.5 to 1.0 to prevent over-sharpening
    UNSHARP_AMOUNT: float = 1.0  # Reduced from 1.5 to 1.0 to preserve fine details
